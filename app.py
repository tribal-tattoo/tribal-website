...
cred = credentials.Certificate(
    "tribal-tattoo-ae837-firebase-adminsdk-em2bs-c5b3a1bd5a.json")
...
app.config['MAIL_PASSWORD'] = 'tribaltattoomysore123'
...
def home():
    ...
    if request.method == 'POST':
        ...
        msg = Message('Booking Confirmation',
                      sender='tribaltattoont@gmail.com', recipients=[email])
...
def booking():
    ...
    if request.method == 'POST':
        ...
        if email != "":
            msg = Message('Booking Confirmation',
                          sender='tribalwebsitemys@gmail.com', recipients=[email])
            msg.body = "Your booking has been registered. We will contact you shortly"
            mail.send(msg)
...
def consult():
    ...
    if request.method == "POST":
        ...
        phNum = request.form['phNum']
        curDate = date.today()
        doc_ref = db.collection(u'consultations').document(
            fullName + "  " + curDate.strftime("%Y-%m-%d"))
        doc_ref.set({
            u'name': fullName,
            u'email': email if email else "",
            u'phNum': phNum
...
def register(event=None):
    ...
    if request.method == 'POST':
        ...
        msg = Message('Registration Confirmation',
                      sender='tribaltattoont@gmail.com', recipients=[email])
        msg.body = "Congratulations, your registration for the event " + \
            event.split(" ")[0] + " on " + \
            event.split(" ")[1] + " has been confirmed"
        mail.send(msg)
        message = "Booking Submitted, Check your mail for confirmation"
